...
class UDocs:
    ...
    def validate_path(self, doc_path):
    ...
    def start(self, path):
        ...
        UDocs.__path = self.validate_path(path)
        self.__userver.router.on(UDocs.__path, 'GET', UDocs.__expose_docs_html, [])
        self.__userver.router.on(UDocs.__path + '/json', 'GET', UDocs.__expose_docs_json, [])